...
def test_loudness_between_tiers_interpolates():
...
def test_peaks_snap_to_bar_height():
...
def test_peaks_decay_after_hold():
    state = VisualizerState()
    state.update([65535, 65535], loudness=32768)
    _ = state.get_spectrum()
    _ = state.get_peaks()
...
def test_peaks_cleared_on_clear():
    state = VisualizerState()
...
def test_render_spectrum_empty_magnitudes():
...
def test_render_spectrum_peak_marker_character():
    magnitudes = [0.5]
    peaks = [0.9]
